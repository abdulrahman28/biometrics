...
namespace WebsocketWinformDemo_Standard
{
    class API_03_GetAttendanceLog
    {
        ...
        private string S_logIDPos;
        /// <summary>
        /// XML字段,
        /// </summary>
        public string S_LogIDPos
        {
            get { return S_logIDPos; }
            set { S_logIDPos = value; }
        }
        #endregion
        ...
        private string logIDBeginPos;
        /// <summary>
        /// XML字段,
        /// </summary>
        public string LogIDBeginPos
        {
            get { return logIDBeginPos; }
            set { logIDBeginPos = value; }
        }
        


        private string[,] log = new string[500, 8];
        /// <summary>
        /// XML字段,
        /// </summary>
        public string[,] Log
        ...
        #region API_03_GetAttendanceLog XML解析Function[方法]
        /// **********************************
        //<?xml version="1.0"?>
        //<Message>
        //<Request>GetAttendanceLog</Request>
        //<Ccid>2</Ccid>
        ...
        //<Action_ext>show/xx</Action_ext>
        //<Response>GetAttendanceLog</Response>
        //<AttendanceLog Count="2" LogIDBeginPos="1">
            //<Log>
                //<LogID>1</LogID>
            ...
            //<Log>
                //<LogID>2</LogID>
                //<Time>2018-05-08-T19:43:19Z</Time>
                //~~~
                //<Photo>No</Photo>
            //</Log>
        //</AttendanceLog>
        ...
        /// <summary>
        /// 解析返回的GetAttendanceLog的数据，解析成功返回true
        /// </summary>
        ...
        public bool F_Response(string ResponseMessage)
        {
            ...
            if (ResponseMessage != null)
            {
                ...
                if (response == "GetAttendanceLog")
                {
                    ...
                    try
                    {
                        var attendanceLog = doc.GetElementsByTagName("AttendanceLog")[0].Attributes;
                        count = attendanceLog.GetNamedItem("Count").Value;
                        logIDBeginPos = attendanceLog.GetNamedItem("LogIDBeginPos").Value;
                    }
                    catch (System.Exception) { }


                    //<----------
                    Int32 countInt = Convert.ToInt32(count);
                    for (int i = 0; i < countInt; i++)
                    {
                        try
                        {
                            log[i, 0] = doc.GetElementsByTagName("LogID")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            log[i, 1] = doc.GetElementsByTagName("Time")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            log[i, 2] = doc.GetElementsByTagName("UserID")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            log[i, 3] = doc.GetElementsByTagName("Action")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            log[i, 4] = doc.GetElementsByTagName("AttendStat")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            log[i, 5] = doc.GetElementsByTagName("APStat")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            log[i, 6] = doc.GetElementsByTagName("JobCode")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            log[i, 7] = doc.GetElementsByTagName("Photo")[i].InnerText;
                        }
                        catch (System.Exception) { }
                    }
                    //---------->
