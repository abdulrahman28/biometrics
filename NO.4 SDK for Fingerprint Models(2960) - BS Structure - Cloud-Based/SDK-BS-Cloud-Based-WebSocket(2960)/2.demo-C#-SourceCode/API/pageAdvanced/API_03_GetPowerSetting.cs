...
{
    class API_03_GetPowerSetting
    {

        #region API_03_GetPowerSetting Attributes[属性]
        ...
        #region API_03_GetPowerSetting XML解析Function[方法]
        /// **********************************
        //<?xml version="1.0"?>
        //<Message>
        //<Request>GetPowerSetting</Request>
        //<Ccid>2</Ccid>
        ...
        //<Action_ext>show/xx</Action_ext>
        //<Response>GetPowerSetting</Response>
        //<Restart>Yes</Restart>
        ...
        /// <summary>
        /// 解析返回的GetPowerSetting的数据，解析成功返回true
        /// </summary>
        ...
        public bool F_Response(string ResponseMessage)
        {
            ...
            if (ResponseMessage != null)
            {
                ...
                if (response == "GetPowerSetting")
                {
                    ...
                    try
                    {
                        restart = doc.SelectSingleNode("/Message/Restart").InnerText;
                    }
                    catch (System.Exception) { }

                    try
                    {
                        //restartTime = Convert.ToDateTime(doc.SelectSingleNode("/Message/RestartTime").InnerText);
                        restartTime = doc.SelectSingleNode("/Message/RestartTime").InnerText;
                    }
                    catch (System.Exception) { }

                    try
                    {
                        sleepAfter = doc.SelectSingleNode("/Message/SleepAfter").InnerText;
                    }
                    catch (System.Exception) { }

                    try
                    {
                        screenSaver = doc.SelectSingleNode("/Message/ScreenSaver").InnerText;
                    }
                    catch (System.Exception) { }

                    try
                    {
                        powerKey = doc.SelectSingleNode("/Message/PowerKey").InnerText;
                    }
                    catch (System.Exception) { }
                    try
                    {
