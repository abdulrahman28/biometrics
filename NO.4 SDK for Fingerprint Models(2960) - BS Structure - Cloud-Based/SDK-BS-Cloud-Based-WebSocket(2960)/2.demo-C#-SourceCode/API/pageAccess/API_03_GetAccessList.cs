...
{
    class API_03_GetAccessList
    {

        #region API_03_GetAccessList Attributes[属性]
        ...
        private string[,] list = new string[50, 9];
        /// <summary>
        /// XML字段,
        /// </summary>
        public string[,] List
        ...
        private string[] list_id  = new string[50];
        /// <summary>
        /// XML字段,
        /// </summary>
        ...
        #region API_03_GetAccessList XML解析Function[方法]
        /// **********************************
        //<?xml version="1.0"?>
        //<Message>
        //<Request>GetAccessList</Request>
        //<Ccid>2</Ccid>
        ...
        //<Action_ext>show/xx</Action_ext>
        //<Response>GetAccessList</Response>
        //<AccessList Count="2">
        //<List ID=”1”>
            //<Use>Yes</Use>
        ...
        /// <summary>
        /// 解析返回的GetAccessList的数据，解析成功返回true
        /// </summary>
        ...
        public bool F_Response(string ResponseMessage)
        {
            ...
            if (ResponseMessage != null)
            {
                ...
                if (response == "GetAccessList")
                {
                    ...
                    try
                    {
                        var accessList = doc.GetElementsByTagName("AccessList")[0].Attributes;
                        count = accessList.GetNamedItem("Count").Value;
                    }
                    catch (System.Exception) { }
                    ...
                    for (int i = 0; i < countInt; i++)
                    {
                        try
                        {
                            var list = doc.GetElementsByTagName("List")[i].Attributes;
                            list_id[i] = list.GetNamedItem("ID").Value;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 0] = doc.GetElementsByTagName("Use")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 1] = doc.GetElementsByTagName("Department")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 2] = doc.GetElementsByTagName("Access")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 3] = doc.GetElementsByTagName("AllWeek")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 4] = doc.GetElementsByTagName("StartWeek")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 5] = doc.GetElementsByTagName("EndWeek")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 6] = doc.GetElementsByTagName("AllDateTime")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 7] = doc.GetElementsByTagName("StartDateTime")[i].InnerText;
                        }
                        catch (System.Exception) { }

                        try
                        {
                            list[i, 8] = doc.GetElementsByTagName("EndDateTime")[i].InnerText;
                        }
                        catch (System.Exception) { }
